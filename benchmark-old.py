...
def basic_eval(query, url, result_format="text/tab-separated-values"):
    ...
    command = ["/usr/bin/curl",  "-D", "/tmp/cheaders", "-o", "/tmp/curl", "-Gs", f"{url}", "--header", f"Accept:{result_format}", "--data-urlencode", f"query={query}"]
...
def process_scholia_benchmark(directory, control_file, skip=0):
    index = 0
...
if args.Scholia:
    process_scholia_benchmark(directory, control_file, skip=skip)
