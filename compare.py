...
parser.add_argument("dirs", nargs="+", help="Directories containing query results")
parser.add_argument("-e", "--engine", default='QLever', help="Engine to use")
...
if args.benchmarks is None:
    ...
    if args.scholia:
        benchmarks = [ 'author', 'award', 'catalogue', 'chemical-class', 'chemical-element', 'chemical', 'clinical_trial', 'complex',
                       'country', 'dataset', 'disease', 'event', 'gene', 'ontology', 'organization', 'pathway',
...
def compare_benchmark(benchmark):
    global tline, texcluded, ttcount, tttime, tsuccess, ttcounts, txcounts, tttimes, tsuccesses
    reference = file(args.engine,benchmark,referencedir)
    if reference is None:
        return
    others = {} 
    excluded = 0 # excluded due to no vocabulary entry
    ...
    while ( line ):
        ...
        if "No vocabulary entry" in line:
            excluded += 1
            note = fields[8] if len(fields) > 8 else ""
            if args.detail:
                print(f"                      {ln:3}        No vocabulary entry {note}")
            for other in others:
                l = others[other].readline() if others[other] else ''
            line = reference.readline()
            ln += 1
            continue
        if fields[5].strip() == "None":
            count = None
        else:
...
#                ttimes[other] += t if c is not None else min(600000, max(t, time * 3)) # moderate penalty
    ...
    print_summary(benchmark, ln-1-excluded, ttime, tcount, success, others, ttimes, tcounts, txcounts, successes)
...
for dir in otherdirs:
    print(f" Fail Time/   Time        Results   Results/", end='   ')
print()
